...
from hades_stats import sp_dist, Distribution

from src.utils.config_tools import load_config
from src.utils.logger import get_logger
from src.utils.data_utils import load_data

def gev_stationnaire(serie: pd.Series) -> Tuple[float, float, float, float]:
    """Renvoie xi, mu, sigma, log-likelihood pour une GEV stationnaire (avec xi = -c)."""
    data = serie.dropna().values
    c, loc, scale = sp_dist('genextreme').fit(data)
    xi = -c
    ll = -sp_dist('genextreme').nnlf((c, loc, scale), data)  # log-likelihood
    return xi, loc, scale, ll

def fit_gev_for_point(key: int, group: pd.DataFrame, col_val: str, len_serie: int=30) -> Tuple[int, Optional[float], Optional[float], Optional[float], Optional[float]]:
    num_poste = key
    serie_valide = group[col_val].dropna()
    if len(serie_valide) < len_serie:
        return num_poste, np.nan, np.nan, np.nan, np.nan
    try:
        xi, loc, scale, ll = gev_stationnaire(serie_valide)
        return num_poste, xi, loc, scale, ll
    except Exception:
        return num_poste, np.nan, np.nan, np.nan, np.nan

def fit_gev_par_point(df_pl: pl.DataFrame, col_val: str, len_serie: int=30, max_workers: int = 48) -> pd.DataFrame:
    """
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(fit_gev_for_point, key, group, col_val, len_serie): key
    ...
    df_result = pd.DataFrame(results, columns=["NUM_POSTE", "xi", "mu", "sigma", "log_likelihood"])
...
def pipeline_gev_from_statisticals(config, max_workers: int = 48):
    ...
    for echelle in echelles:
        ...
        if model == "config/modelised_settings.yaml":
            ...
            max_calcul_year = 32 if echelle == "horaire" else max_year - min_year + 1 # nombre maximal possible
            for len_serie in range(min_calcul_year, max_calcul_year + 1):
                df_gev_param = fit_gev_par_point(df, mesure, len_serie=len_serie, max_workers=max_workers)
                df_gev_param.to_parquet(f"{output_dir}/gev_param_years_{len_serie}.parquet")
                logger.info(f"Enregistré sous {output_dir}/gev_param_years_{len_serie}.parquet")

...
"""
Module complet pour :
1) Télécharger les fichiers ZIP (horaires et quotidiens) depuis meteo.data.gouv.fr
2) Générer des fichiers NetCDF (par année) à partir de ces fichiers ZIP.

Workflow principal :
    1. Téléchargement et stockage dans :
       - data/horaire_zip/ (pour l'échelle horaire)
       - data/quotidien_zip/ (pour l'échelle quotidienne)
    2. Parcours des ZIP (2 passes) :
       Pass 1 : détecte toutes les stations existantes
       Pass 2 : parse les CSV, répartit par année (1959-2022), station, date => NetCDF.
    3. Chaque NetCDF (observed_YYYY01010000-YYYY12312359.nc) contient :
       - Dimension time (axe complet de l'année)
       - Dimension station (toutes les stations détectées)
       - Variable pr (remplie ou NaN)
    4. Nettoyage : Les ZIP sont supprimés après le traitement.
"""
...
def download_quotidien_zip(dep: int, output_dir: Path, logger) -> None:
    ...
    if output_file.exists():
        logger.info(f"Fichier quotidien déjà présent pour le département {dep:02d}")
        return

    try:
        logger.info(f"Téléchargement quotidien : {url}")
        r = requests.get(url, stream=True)
        if r.status_code == 200:
            with open(output_file, "wb") as f:
                for chunk in r.iter_content(chunk_size=8192):
                    f.write(chunk)
            logger.info(f"Téléchargement terminé pour le département {dep:02d}")
        else:
            logger.warning(f"Impossible de télécharger {url} (status={r.status_code})")
...
def download_horaire_zip(dep: int, output_dir: Path, logger) -> None:
    """
    Télécharge dynamiquement tous les fichiers horaires disponibles pour un département donné
    depuis la page : https://meteo.data.gouv.fr/datasets/6569b51ae64326786e4e8e1a
    """
    from bs4 import BeautifulSoup

    base_html = "https://meteo.data.gouv.fr/datasets/6569b51ae64326786e4e8e1a"
    base_prefix = "https://object.files.data.gouv.fr/meteofrance/data/synchro_ftp/BASE/HOR"

    try:
        response = requests.get(base_html)
        if response.status_code != 200:
            logger.warning(f"Impossible d'accéder à la page HTML {base_html} (status={response.status_code})")
            return
    except Exception as e:
        logger.error(f"Erreur lors de l'accès à {base_html} : {e}")
        return

    soup = BeautifulSoup(response.content, "html.parser")
    links = soup.find_all("a")

    target_prefix = f"H_{dep:02d}_"

    for link in links:
        href = link.get("href")
        if not href or not href.endswith(".csv.gz"):
            continue
        if target_prefix in href:
            full_url = f"{base_prefix}/{href}"
            output_file = output_dir / href

            if output_file.exists():
                logger.info(f"Fichier horaire déjà présent : {href}")
                continue

            try:
                logger.info(f"Téléchargement horaire : {full_url}")
...
def download_all_zips(echelles: list, logger) -> None:
    """
    Télécharge tous les ZIP pour les échelles demandées (horaire / quotidienne)
    et les stocke dans data/horaire_zip ou data/quotidien_zip.
    """

    horaire_dir = Path("data/horaire_zip")
    quotidien_dir = Path("data/quotidien_zip")
    horaire_dir.mkdir(parents=True, exist_ok=True)
    quotidien_dir.mkdir(parents=True, exist_ok=True)

    # On boucle sur les départements 1..95
    # (en France métropole, hors Corse 2A/2B, DOM, etc.)
    for dep in range(1, 96):
        if "quotidienne" in echelles:
            download_quotidien_zip(dep, quotidien_dir, logger)
        if "horaire" in echelles:
            download_horaire_zip(dep, horaire_dir, logger)
...
def get_time_axis(annee: int, echelle: str) -> pd.DatetimeIndex:
    ...
    if echelle == "horaire":
        ...
        end = f"{annee}-12-31 23:00:00"  # dernière heure = 23:00
...
def parse_csv(echelle: str, csv_path: Path, station_index: dict) -> pd.DataFrame:
    """
    Lit un fichier CSV (colonnes : NOM_USUEL, LAT, LON, AAAAMMJJ[HH], RR ou RR1)
...
def pipeline_obs_from_zip_to_nc(config):
    ...
    # 1) Téléchargement
    logger.info("[DL] Début du téléchargement de tous les ZIP.")
    download_all_zips(echelles, logger)
    logger.info("[DL] Téléchargement terminé.")
    ...
    # Dossiers ZIP
    zip_dirs = {
        "horaire": Path("data/horaire_zip"),
        "quotidienne": Path("data/quotidien_zip")
    }

...
for config in ["config/modelised_settings.yaml"]: # "config/observed_settings.yaml", 
    ...
    if config == "config/modelised_settings.yaml":
        ECHELLES = ["w3", "w6", "w9", "w12", "w24"] # Pas de temps horaire uniquement pour AROME
        # "horaire", 
    else:
        ECHELLES = ["w3", "w6", "w9", "w12", "w24"]
        # "horaire" "quotidien"
...
#                 # "ns_gev_m1_break_year", "ns_gev_m2_break_year", 
#                 "ns_gev_m3_break_year" # Non stationnaire avec point de rupture
#                 ]:
...
#                         "--season", season,
#                         "--model", model,
#                         # Bootstrap uniquement sur M3_break
#                         "--model_bootstrap", str(True) if model =="ns_gev_m3_break_year" else str(False)
#                     ],
#                     check=True
#                 )

#             for method in ["aic", "lrt"]:
#                 subprocess.run(
#                     [
#                         "python",
#                         "-m",

...
## Avec 
- $X \sim \mathrm{GEV}(\mu, \sigma, \xi)$ avec $F^{-1}$ la fonction quantile associée
- Le niveau de retour $z_T$ de la période $T$

# Méthodologie du calcul de tendance

Pour $\mathbf t$ une année normalisée :

$$
z_T({\mathbf t}) =
\begin{cases}
\mu_0+\mu_1\,{\mathbf t}
+\frac{\sigma_0+\sigma_1\,{\mathbf t}}{\xi}\,\left[(-\ln\!\left(1-\frac{1}{T}\right)\right)^{-\xi}-1\big] & \text{si } \xi \ne 0 \text{ et } T>1 \\
\mu_0+\mu_1\,{\mathbf t}
-(\sigma_0+\sigma_1\,{\mathbf t})
\ln\!\left(-\ln\!\left(1-\frac{1}{T}\right)\right) & \text{si } \xi = 0 \quad \text{(Gumbel)}
\end{cases}
$$
...
\fontsize{7}{7}\selectfont Station Météo-France de Saint-Marcel-lès-Valence, Vallée du Rhône ($M_2$, $M_3$, $M_3$)

![](schema/26313001_serie_fev.png)

# Méthodologie du calcul de tendance

\fontsize{7}{7}\selectfont Station Météo-France de Saint-Marcel-lès-Valence, Vallée du Rhône ($M_1^\ast$, $M_2$, $M_2$)

![](schema/26313001_serie_nov.png)

...
shutil.rmtree(".jupyter_cache", ignore_errors=True)
...
def pipeline_total_title_map_legend_scatter_delta(
    ...
    title_show = pipeline_title(title, year_min, year_max, echelle)
...
echelle = "quotidien"
season = "hydro"
title = f"Modèle minimisant l'AIC (SAISON {season})"
...
```{python}
#| label: niveau_retour_20_ans_sans_effet_temporel
echelle = "quotidien"
unit_choice = "mm/j"
...
title = f"Niveau de retour {T_choice} ans sans effet temporel"
pipeline_total_title_map_legend_scatter_delta(
    title=title,
    year_min=min_year_choice,
    year_max=max_year_choice,
    echelle=echelle,
    html_map_legend=html_map_legend,
    html_scatter=html_scatter,
    me=me,
...
def get_year_range(model: str, min_year: int, max_year: int) -> int:
    if "break_year" in model:
        return max_year - 1985
    else:
        return max_year - min_year
```

```{python}
#| label: changement_niveau_retour_par_10_ans
# -------------------- CONFIGURATION --------------------
T_choice = 20
...
# Unités selon l'échelle
unit = "mm_h" if echelle == "horaire" else "mm_j"
models = [v for v in model_options.values() if v != "s_gev"]

min_year, max_year = config["years"]["min"], config["years"]["max"]
mod_dir = Path(config["gev"]["modelised"]) / echelle
obs_dir = Path(config["gev"]["observed"]) / echelle

# -------------------- BOUCLE D’AFFICHAGE PAR MODÈLE --------------------
for model in models:
    year_range = get_year_range(model, min_year, max_year)

    def load_and_prepare(df_path, typ):
        df = pl.read_parquet(df_path)
        df = filter_nan(df, "xi")
        df = add_metadata(df, unit, type=typ)
        df = ensure_mu1_sigma1(df)
        return df

    df_mod = load_and_prepare(mod_dir / f"gev_param_{model}.parquet", "modelised")
    df_obs = load_and_prepare(obs_dir / f"gev_param_{model}.parquet", "observed")

    # Expressions Polars pour ∂qT/∂année
    mu1_nat_expr = pl.col("mu1") / year_range
    sigma1_nat_expr = pl.col("sigma1") / year_range
    C_T_expr = (log_term ** (-pl.col("xi"))) - 1
    delta_expr = ((mu1_nat_expr + (sigma1_nat_expr / pl.col("xi")) * C_T_expr) * 10).alias(column_to_show)

    df_mod = df_mod.with_columns([delta_expr])
    df_obs = df_obs.with_columns([delta_expr])

    df_mod_show = dont_show_extreme(df_mod, column_to_show, quantile_choice, stat_choice_key=None)
    df_obs_show = dont_show_extreme(df_obs, column_to_show, quantile_choice, stat_choice_key=None)

    result = {
        "modelised": df_mod,
        "modelised_show": df_mod_show,
        "observed": df_obs,
        "observed_show": df_obs_show,
        "column": column_to_show,
        "echelle": "diverging_zero_white",
        "continu": True
    }

    _, _, r2, me, _, percent = pipeline_map_legend_scatter(
    ...
    display(HTML(f"""
        <div style="font-size: 18px; color: #333; margin-bottom: 15px;">
            <p style="font-size: 20px; font-weight: bold; color: #2c3e50; margin-bottom: 5px;">
                {model_labels[model]}
            </p>
            <p style="font-size: 17px; color: #3498db; margin: 0;">
                <strong>r²</strong> = {r2:.3f} &nbsp;|&nbsp;
                <strong>ME</strong> = {me:+.3f} {unit} &nbsp;|&nbsp;
                <strong>Δ</strong> = {percent * 100:+.1f}%
            </p>
        </div>
    """))
```
...
```{python}
model = "ns_gev_m1"
title = f"Changement du niveau de retour par 10 ans du modèle {model_labels[model]}"
display(HTML(f"""
    <div style="font-size: 18px; color: #333;">
        <p style="font-size: 22px; font-weight: bold; color: #2c3e50;">{title}</p>
        <p style="font-size: 18px; color: #3498db;">
...
val_max = max(df_mod_show[column_to_show].max(), df_obs_show[column_to_show].max())
val_min = min(df_mod_show[column_to_show].min(), df_obs_show[column_to_show].min())
...
html_map_legend, html_scatter, r2, me, _, percent = pipeline_map_legend_scatter(
    name=f"changement_retour_{T_choice}_{model}",
    result=result,
    echelle=echelle,
    stat_choice_label=""

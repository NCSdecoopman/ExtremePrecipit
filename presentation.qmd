...
def plot_four_polars_grids(
    ...
    *,
    ...
    deps_metro["geometry"] = deps_metro.simplify(500)
    ...
    for ax, gdf_m, gdf_o, title in zip(axes, model_gdfs, obs_gdfs, titles):
        ...
        relief_gdf.simplify(100)

        ax.set_title(title, fontsize=11, pad=1)
        ax.set_axis_off()

        # Choix de la couche à rasteriser
        order_raster = 2 if is_bool else 1     # ← clé du comportement
        ax.set_rasterization_zorder(order_raster)

    # --- 8) Barre de couleurs commune --------------------------------
    sm = mpl.cm.ScalarMappable(cmap=cmap, norm=norm)
    sm.set_array([])
    cbar = fig.colorbar(sm, ax=axes.ravel().tolist(), shrink=0.4, aspect=25)

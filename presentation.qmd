...
![](presentation/temperature.png){max-height=10px}
...
def plot_four_polars_grids(
    ...
    *,
    ...
    if all_values and not is_bool:
        seuil = np.percentile(np.abs(all_values), staturation_val)
    ...
    # 
    # ------------------------------------------------------------------
    # Définir (ou exposer via les paramètres) un intervalle de tailles en points²
    min_ms, max_ms = 20, 150          # ex. diamètres ~4 pt à ~12 pt
    abs_max = max(                    # normalisation sur l’ensemble des obs
        g[val_col].abs().max()
        for g in obs_gdfs
    ...
    for title, gdf_m, gdf_o in zip(titles, model_gdfs, obs_gdfs):
        for mode, z in modes.items():
            ...
            if show_obs and gdf_o is not None and not gdf_o.empty:
                # Diamètre linéaire entre 4 pt et 13 pt
                ms = (gdf_o[val_col].abs() / 50).clip(0, 1) * (max_pt - min_pt) + min_pt
                ms_scatter = ms ** 2              # aire en points²

                kw = dict(
                    markersize=ms_scatter,        # <-- utiliser ms_scatter ici
                    marker="o",
...
```{python}
title_show = pipeline_title("Tendances relatives (%) du niveau de retour 10 ans estimée par le meilleur modèle", 1985, 2022, "horaire", None)
display(HTML(title_show))
```
  ...
  <tbody>
    <tr>
        ...
        <a href="presentation_files/map/horaire_SON_rast_mod.svg" target="_blank" rel="noopener">
            <img src="presentation_files/map/horaire_SON_norast_mod.svg" alt="SON">
        </a>
        </td>
        <td>
        <a href="presentation_files/map/horaire_DJF_rast_mod.svg" target="_blank" rel="noopener">
            <img src="presentation_files/map/horaire_DJF_norast_mod.svg" alt="DJF">
        </a>
        </td>
        <td>
        <a href="presentation_files/map/horaire_MAM_rast_mod.svg" target="_blank" rel="noopener">
            <img src="presentation_files/map/horaire_MAM_norast_mod.svg" alt="MAM">
        </a>
        </td>
        <td>
        <a href="presentation_files/map/horaire_JJA_rast_mod.svg" target="_blank" rel="noopener">
            <img src="presentation_files/map/horaire_JJA_norast_mod.svg" alt="JJA">
        </a>
        </td>
    <td rowspan="2" style="vertical-align: middle; text-align: center;">
    <object type="image/svg+xml" data="presentation_files/map/horaire_legend.svg"
            width="40" height="600" class="legend-svg">
    </object>
    </td>
    </tr>
    <tr>
      <td>
        <a href="presentation_files/map/horaire_SON_rast_obs.svg" target="_blank" rel="noopener">
          <img src="presentation_files/map/horaire_SON_norast_obs.svg" alt="SON">
        </a>
      </td>
      <td>
        <a href="presentation_files/map/horaire_DJF_rast_obs.svg" target="_blank" rel="noopener">
          <img src="presentation_files/map/horaire_DJF_norast_obs.svg" alt="DJF">
        </a>
      </td>
      <td>
        <a href="presentation_files/map/horaire_MAM_rast_obs.svg" target="_blank" rel="noopener">
          <img src="presentation_files/map/horaire_MAM_norast_obs.svg" alt="MAM">
        </a>
      </td>
      <td>
        <a href="presentation_files/map/horaire_JJA_rast_obs.svg" target="_blank" rel="noopener">
          <img src="presentation_files/map/horaire_JJA_norast_obs.svg" alt="JJA">
        </a>
      </td>
    </tr>
  </tbody>

...
- Données du CP-RCM CNRM-AROME et données stations Météo-France
...
axes[0].set_title("Effet de $\\mu$", loc='right')
axes[0].set_xlabel("x")
...
axes[1].set_title("Effet de $\\sigma$", loc='right')
axes[1].set_xlabel("x")
...
axes[2].set_title("Effet de $\\xi$", loc='right')
axes[2].set_xlabel("x")
...
::: {.revealjs-slide background-transparent=true}

```{=html}
<script>
function reloadIframes(slide) {
  const iframes = slide.querySelectorAll('iframe');
  for (const iframe of iframes) {
    const src = iframe.src;
    // Skip empty iframes or already reloaded
    if (!src) continue;
    iframe.src = '';  // Unload
...
Reveal.on('ready', event => reloadIframes(event.currentSlide));

// À chaque transition (plus fiable que slidechanged)
Reveal.on('slidetransitionend', event => reloadIframes(event.currentSlide));
</script>

...
year_display_middle, height, unit_choice = 1990, 600, "mm/j"
...
afficher_cat = [
    ...
    "1h", "3h glissante", "6h glissante", "9h glissante", "12h glissante", "24h glissante", "1h (agreg 3x3)"
    ]
...
fig_r2_all_max.update_traces(
    textfont=dict(size=6),  # réduit la taille du texte à 10 px
...
```{python}
#| label: minimize_gev_pval_map
echelle = "quotidien"
season = "son"
title = f"Modèle minimisant la p-value"
mod_dir = Path(config["gev"]["modelised"]) / echelle / season
obs_dir = Path(config["gev"]["observed"]) / echelle / season

df_model_pval = pl.read_parquet(mod_dir / "gev_param_best_model_lrt.parquet")
df_obs_pval = pl.read_parquet(obs_dir / "gev_param_best_model_lrt.parquet")

df_model_pval = add_metadata(df_model_pval, "mm_h" if echelle == "horaire" else "mm_j", type="modelised")
df_obs_pval = add_metadata(df_obs_pval, "mm_h" if echelle == "horaire" else "mm_j", type="observed")

df_model_pval = filter_nan(df_model_pval, "xi") # xi est toujours valable
df_obs_pval = filter_nan(df_obs_pval, "xi") # xi est toujours valable

df_model_pval = df_model_pval.with_columns(
    pl.col("model").replace(model_labels)  # remplace par noms lisibles
)
df_obs_pval = df_obs_pval.with_columns(
    pl.col("model").replace(model_labels)
)

result = {
    "modelised": df_model_pval,
    "modelised_show": df_model_pval,
    "observed": df_obs_pval,
    "observed_show": df_obs_pval,
    "column": "model",
    "echelle": "diverging_zero_white",
    "continu": False,
    "categories": list(model_labels.values())  # ["M₁(μ₀, σ₀)", "M₂(μ, σ₀)", ...]
...
df_model_pval = df_model_pval.with_columns(pl.col("NUM_POSTE").cast(pl.Int32))
df_obs_pval = df_obs_pval.with_columns(pl.col("NUM_POSTE").cast(pl.Int32))
df_obs_vs_mod_full = pl.read_csv(f"data/metadonnees/obs_vs_mod/obs_vs_mod_{echelle}.csv")
...
for param in params:
    obs_vs_mod = match_and_compare(df_obs_pval, df_model_pval, param, df_obs_vs_mod_full)
...
# Sauvegarde et affichage dans un iframe
scatter_path = f"assets/hist_r2_jour_model_min_pval.html"
pio.write_html(fig_r2_all_max, file=scatter_path, include_plotlyjs="cdn", full_html=False)
...
```{python}
model_name="ns_gev_m3_break_year"
echelle="quotidien"
season_choice = "djf"
model_title="M₃⋆(μ, σ)" # M ₀, ₁,₂, ₃, ⋆
half = "sud"  # ou "nord"
...
```{python}
model_name="ns_gev_m3_break_year_bootstrap"
echelle="quotidien"
season_choice = "djf"
model_title="M₃⋆ᵇ(μ, σ)" # M ₀, ₁,₂, ₃, ⋆
half = "sud"  # ou "nord"
carte_retour(model_name, echelle, season_choice, half)
```

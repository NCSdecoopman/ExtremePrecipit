...
def menu_statisticals(min_years: int, max_years: int, STATS, SEASON):
    ...
    with col4:
        if st.session_state["stat_choice"] in ["Cumul", "Jour de pluie"]:
            ...
            st.selectbox("Choix de l'échelle temporelle", ["Journalière", "Horaire", "horaire_aggregate_n3", "horaire_aggregate_n5",
...
def menu_gev(config: dict, model_options: dict, ns_param_map: dict, SEASON, show_param: bool):
    ...
    if st.session_state["model_type"] is not None:
        ...
        if selected_model != "Niveau de retour":
            # Paramètre GEV
            with col4:
                if show_param:
        ...
        if st.session_state["run_analysis"]:
            ...
            return {    
                ...
                "param_choice": st.session_state["param_choice"] if selected_model != "Niveau de retour" else None,
                "param_choice_pres": selected_label if selected_model != "Niveau de retour" else None,
                ...
                "mod_dir": mod_dir,
                "obs_dir": obs_dir,
                "T_choice": st.session_state["T_choice"] if selected_model != "Niveau de retour" else 10,
                "par_X_annees": st.session_state["par_X_annees"] if selected_model != "Niveau de retour" else 10
            }

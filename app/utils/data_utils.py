...
def load_data(type_data: str, echelle: str, min_year: int, max_year: int, season: str, col_to_load: str, config) -> pl.DataFrame:
    ...
    for year in range(min_year, max_year + 1):
        try:
            ...
            # Uniformisation des types pour éviter le SchemaError :
            #    - NUM_POSTE toujours Int64
            #    - toutes les autres colonnes chargées (stat, nan_ratio…) toujours Float64
            cols_to_cast = [c for c in col_to_load if c != "NUM_POSTE"]
            casts = [
                pl.col("NUM_POSTE").cast(pl.Int64),

...
def prepare_layer(df: pl.DataFrame) -> pl.DataFrame:
    ...
    if "de_significant" in df.columns:
...
def create_layer(df: pl.DataFrame) -> pdk.Layer:
    ...
    if "de_significant" in df.columns:
        st.write(f"Pourcentage de True AROME : {df["significant"].mean() * 100:.2f}%")
        df_sig = df.filter(pl.col("significant"))
        df_non_sig = df.filter(~pl.col("significant"))
    else:
        df_sig = pl.DataFrame()
        df_non_sig = df
    ...
    if len(df_sig) > 0:
        df_sig = df_sig.with_columns(pl.lit("X").alias("star_text"))
...
def create_scatter_layer(df: pl.DataFrame, radius=1500) -> list[pdk.Layer]:
    ...
    if "de_significant" in df.columns:
        st.write(f"Pourcentage de True OBS : {df["significant"].mean() * 100:.2f}%")
        df_sig = df.filter(pl.col("significant"))
        df_non_sig = df.filter(~pl.col("significant"))
    else:
        df_sig = pl.DataFrame()

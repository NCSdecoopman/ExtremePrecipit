...
return {
  {
    ...
    Meta = function(meta)            
      if quarto.doc.is_format("pdf") then
        ...
        local volume = meta['volume']
        local issue = meta['issue']
        local year = meta['year']
        local month = meta['month']
        local doi = meta['doi']
        
        if volume ~= nil then
          meta['volume-latex'] = pandoc.RawBlock('tex', '\\Volume{' .. pandoc.utils.stringify(volume) .. '}')
        end
        
        if issue ~= nil then
          meta['issue-latex'] = pandoc.RawBlock('tex', '\\Issue{' .. pandoc.utils.stringify(issue) .. '}')
        end
        
        if year ~= nil then
          meta['year-latex'] = pandoc.RawBlock('tex', '\\Year{' .. pandoc.utils.stringify(year) .. '}')
        end
        
        if month ~= nil then
          meta['month-latex'] = pandoc.RawBlock('tex', '\\Month{' .. pandoc.utils.stringify(month) .. '}')
        end
        
        if doi ~= nil then
          meta['doi-latex'] = pandoc.RawBlock('tex', '\\DOI{' .. pandoc.utils.stringify(doi) .. '}')
        end
      end

...
def plot_series_station(station_id):
    ...
    # Ficher z_T_p
    file_1990_2022 = Path('data/gev/observed/horaire/fev/niveau_retour.parquet')
    file_1959_2022 = Path('data/gev/observed/quotidien/fev/niveau_retour.parquet')
    zTp_1959_2022 = pl.read_parquet(file_1959_2022)
    zTp_1990_2022 = pl.read_parquet(file_1990_2022)

    # Fichiers Parquet GEV
    file_1990_2022 = Path('data/gev/observed/horaire/fev/gev_param_best_model.parquet')
    file_1959_2022 = Path('data/gev/observed/quotidien/fev/gev_param_best_model.parquet')
    ...
    df_1959_2022 = load_data(input_dir_quot, "fev", "quotidien", cols_quot, 1959, 2022)
    df_1990_2022 = load_data(input_dir_hor, "fev", "horaire", cols_hor, 1990, 2022)
    ...
    file_quot_reduce = Path('data/gev/observed/quotidien_reduce/fev/gev_param_best_model.parquet')
    ...
    df_quot_reduce = load_data(input_dir_quot_reduce, "fev", "quotidien_reduce", cols_quot_reduce, 1990, 2022)
    ...
    df_quot_1990_2022 = load_data(input_dir_quot, "fev", "quotidien", cols_quot_1990, 1990, 2022)
    ...
    df_series_quot_1990_2022 = build_x_ttilde(df_quot_1990_2022, 1990, 2022, gev_quot_reduce, break_year, mesure_quot)
    ...
    df_station_quot_reduce = df_quot_reduce.filter(pl.col("NUM_POSTE") == station_id_cast)
    years_quot_reduce = df_station_quot_reduce["year"].to_numpy()
    values_quot_reduce = df_station_quot_reduce[mesure_quot_reduce].to_numpy()
    ...
    ax1.set_title("Données journalières (1959-2022)")
    ...
    ax2.plot(years_hor, values_hor, label="Série observée horaire", color="black", marker="o", linestyle="-", alpha=0.5)
    ax2.plot(years_1990_2022, z_1990_2022, label="Niveau de retour 1990-2022", color="red")
    ax2.set_ylabel("Précipitation (mm/h)")
    ax2.set_title("Données horaires (1990-2022)")
    ...
    ax2.scatter([1995, 2022], [z_1990_2022[1995-1990], z_1990_2022[-1]], s=40, color='red', zorder=5)
    ax2.text(
        0.02, 0.88, f"Tendance : {trend_1990:.1f}%", transform=ax2.transAxes,
        color='red', fontsize=12, fontweight='bold', va='top', ha='left'
    )
    ax2.set_xlabel("Année")

    # --- Facette 3 : Données journalières (1990-2022) ---
    ax3.plot(years_quot_1990_2022, values_quot_1990_2022, label="Série observée quotidienne", color="black", marker="o", linestyle="-", alpha=0.5)
    ax3.plot(years_quot_reduce_grid, z_quot_reduce, label="Niveau de retour 1990-2022", color="orange")
    ax3.set_ylabel("Précipitation (mm/j)")
    ax3.set_title("Données journalières (1990-2022)")
    ...
    ax3.scatter([1995, 2022], [z_quot_reduce[1995-1990], z_quot_reduce[-1]], s=40, color='orange', zorder=5)
    ax3.text(
        0.02, 0.88, f"Tendance : {trend_reduce:.1f}%", transform=ax3.transAxes,
    ...
    fig.savefig(output_dir / f"{station_id}_serie.png", dpi=150)
    plt.close(fig)

if __name__ == "__main__":
    plot_series_station("30352002")

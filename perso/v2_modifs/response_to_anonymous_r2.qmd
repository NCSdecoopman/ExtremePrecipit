...
As a sensitivity check, we computed relative trends in $RL_2$ and $RL_5$ using the same non-stationary GEV models and the same 1992–2022 trend window as for $RL_{10}$ (@fig-rl-daily, @fig-rl-hourly). Quantitative comparisons (hydrological year, all stations with a selected GEV model) show that the **spatial patterns are robust to the choice of return period**, while trend **magnitudes** increase with $T$, as expected for higher quantiles:
...
We note that significance is assessed from the $RL_{10}$ trend uncertainty (10% level); the same 704 daily and 193 hourly stations are therefore flagged across return periods. This is conservative for $RL_2$ and $RL_5$, which are less noisy than $RL_{10}$ at the hourly scale. Together with the annual-maxima vs.\ $RL_2$ check in our response to Reviewer #1 ($r=0.88$ for relative trends across 1,558 daily stations), these results indicate that the trend maps (Figures 6 and 7) are not driven by GEV fitting artefacts or by the choice of a high return period alone.

We retain $RL_{10}$ as the primary metric because it is a standard hydrological design threshold and because our daily (63-year) and hourly (33-year) records are sufficient for its estimation. In addition, using the same return period at both daily and hourly scales enables a direct and consistent comparison of the extreme trend characteristics between the two time resolutions. The results for $RL_2$ and $RL_5$ confirm the robustness of the detected spatial patterns. We present the lower-return-period maps here for the reviewer's assessment rather than as an additional manuscript appendix, to keep the main text focused.

\clearpage

![Relative GEV trends of $\mathrm{RL}_2$ (first row) and $\mathrm{RL}_5$ (second row) for daily precipitation (hydrological year): columns are AROME (left) and Météo-France stations (right), with a separate colour scale for each row. Same display conventions as Figures 6 and 7: open circles for non-significant stations ; filled grey for significant near-zero trends; coloured markers for significant trends (uniform size); non-significant AROME grid cells not displayed.](Figures/rl2_rl5_trends_daily.png){#fig-rl-daily width=95%}

\clearpage

![Relative GEV trends of $\mathrm{RL}_2$ (first row) and $\mathrm{RL}_5$ (second row) for hourly precipitation (hydrological year): columns are AROME (left) and Météo-France stations (right), with a separate colour scale for each row. Same display conventions as Figures 6 and 7: open circles for non-significant stations ; filled grey for significant near-zero trends; coloured markers for significant trends (uniform size); non-significant AROME grid cells not displayed.](Figures/rl2_rl5_trends_hourly.png){#fig-rl-hourly width=95%}
...
The Figure 4 caption has been updated for uniform marker size and zero-bias display; captions for Figures 6 and 7 and Appendix C describe the significance conventions above. The revised maps are shown in our response to Reviewer #1 (Comment 7): Figure 4 (climatology) and Figures 6 and 7 (trends).

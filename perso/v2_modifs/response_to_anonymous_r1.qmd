...
- In `Section 4.2.2 (Hourly)` (at the very beginning):
  > _"Similarly to the daily scale, we verified the consistency of GEV hourly trends by comparing GEV-estimated mean trends to linear regression trends of observed annual maxima. Despite the shorter record length and the higher noise level of sub-daily extremes, we find a robust agreement (Pearson correlation $r = 0.72$ for absolute trends, $r = 0.70$ for relative trends)."_

\clearpage

![Comparison of the spatial distribution of relative daily trends (%) over 1960–2022 across all 1,558 daily stations: (a) observed annual maxima trend obtained by simple linear regression; (b) GEV-estimated mean trend.](Figures/comparison_maxima_gev_maps_daily.png){#fig-maxima-gev-maps-daily width=62%}

![Scatter plots of GEV-estimated daily mean trend vs. observed daily annual maxima trend across all 1,558 daily stations: (a) absolute trends in mm/decade; (b) relative trends in %. The 1:1 line is shown in dashed grey, and the linear regression fit is in red, with Pearson correlation coefficients ($r$) indicated.](Figures/comparison_maxima_gev_scatter_daily.png){#fig-maxima-gev-scatter-daily width=60%}

\clearpage

![Comparison of the spatial distribution of relative hourly trends (%) over 1990–2022 across all 562 hourly stations: (a) observed annual maxima trend obtained by simple linear regression; (b) GEV-estimated mean trend.](Figures/comparison_maxima_gev_maps_hourly.png){#fig-maxima-gev-maps-hourly width=62%}

![Scatter plots of GEV-estimated hourly mean trend vs. observed hourly annual maxima trend across all 562 hourly stations: (a) absolute trends in mm/decade; (b) relative trends in %. The 1:1 line is shown in dashed grey, and the linear regression fit is in red, with Pearson correlation coefficients ($r$) indicated.](Figures/comparison_maxima_gev_scatter_hourly.png){#fig-maxima-gev-scatter-hourly width=60%}
...
Percentage of locations where each model category is selected as the best fit for daily precipitation extremes ($M_0$ representing no trend, $M$ a linear trend, and $M^*$ a breakpoint trend starting in 1985). All percentages sum to 100\% for each dataset.
:::
...
- In `Section 4.2.1 (Daily)`, we have added a sentence stating the percentage of stations and grid points where $M_0$, $M$, and $M^*$ models were selected for the hydrological year as an example (66.1%, 17.8%, and 16.0% for observed stations; 75.5%, 11.8%, and 12.7% for AROME, respectively).
...
* **Response:** We agree. The subsection defining the agreement metrics (Pearson correlation $r$, Mean Error $ME$, and relative bias in %) and introducing all the evaluated variables (mean number of wet days, mean annual total precipitation, daily and hourly 10-year return levels) was indeed commented out in the draft.
...
- In the same section, we have added a second panel to Figures 6 showing the seasonal evolution of the relative bias (in \%) for the four precipitation indices.
...
1. In `Section 4.1 (Evaluation of the precipitation climatology of AROME)`, we have added a fourth row to the climatology maps (Figure 4) to present the relative bias (%) alongside the absolute difference in the third row, making the regional comparisons clearer while preserving the absolute magnitude information.

2. In all map figures across the manuscript (Figure 4 in `Section 4.1`, Figures 6 and 7 in `Sections 4.2`/`4.3`, and the monthly trend maps in `Appendix C`), we have used a uniform dot size for all displayed stations so that larger dots do not obscure adjacent stations.

3. In `Section 4.1 (Evaluation of the precipitation climatology of AROME)`, the color scale of the climatology maps (Figure 4) has been modified so that a zero bias is represented by a clearly visible grey fill.
...
## Comment 8: Seasonal Bias Synthesis (Figures 6)
...
- In `Section 4.1 (Evaluation of the precipitation climatology of AROME)`, Figures 6 (seasonal synthesis) has been updated to include a second panel (b) presenting the seasonal relative bias (in %) for the four precipitation indices, and the caption has been updated to describe both panels.

![](fig4.pdf){width=100%}
_*Figures 6.* Seasonal synthesis of the climatological agreement between the AROME model and Météo-France stations for the four precipitation indices: (a) spatial Pearson correlation ($r$) and (b) relative bias ($RB$, in \%) depending on the season._
...
- In `Section 4.2.2 (Hourly)`, the section describing monthly trends has been rewritten to be more quantitative, referencing the new Figure 8 to detail the median trends, relative biases, and spatial correlations:
  ...
  >
  > This systematic underestimation is reflected in the monthly relative biases ($RB$) between the corresponding AROME grid points and stations shown in Figure 8b, with large negative values at the hourly scale during convective months (e.g., $RB = -152.0\%$ in February, $RB = -64.7\%$ in March, and $RB = -137.5\%$ in June), whereas daily relative biases remain much smaller (e.g., $RB = -2.7\%$ in January and $RB = -43.7\%$ in March).
  >

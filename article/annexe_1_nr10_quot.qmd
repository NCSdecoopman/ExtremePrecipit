...
for s in seasons:
    ...
    assemble_vertical(base/s/"mod_rast.svg", base/s/"obs_rast.svg", base/"legend.svg", Path(f"figures/nr10_{echelle}_{s}.svg"))
    assemble_un(base/s/"obs_rast_diff.svg", base/"legend_diff.svg", Path(f"figures/nr10_{echelle}_{s}_diff.svg"))

combined_nr = load_metrics_csv(f"../outputs_nr10/maps/gev_zTpa/{echelle}/{comp}/{sat}/metrics.csv")

# Macros
macros = ""
for s in seasons:
    row = season_row(combined_nr, s)
    r, n, me, d = float(row["r"]), int(row["n"]), float(row["me"]), float(row["delta"])

...
def plot_r_by_season(
    ...
    for ax, col_value in zip(axes, col_values):
        ...
        # ----------------------------------------------------
        # PRINT des barres : valeurs r + % (si dispo)
        # ----------------------------------------------------
        print("\n" + "="*90)
        print(f"[{nom_fichier}] col_value = {col_value} | echelles = {echelles} | col_plot = {col_plot}")
        print("- Values (each bar):")
        print(pivot.round(3).to_string())

        if pivot_pct is not None:
            print("\n- % stations (same bars):")
            # si pct <= 1 => afficher en %
            pct_disp = pivot_pct.copy()
            pct_disp = pct_disp.applymap(lambda v: (v*100) if pd.notna(v) and v <= 1.0 else v)
            print(pct_disp.round(0).to_string())
        print("="*90 + "\n")
...
selected_seasons = ["jan", "fev", "mar", "avr", "mai", "jui", "juill", "aou", "sep", "oct", "nov", "dec"]
to_plot = combined_all[combined_all["season"].isin(selected_seasons)]
plot_r_by_season(to_plot , echelles=['quotidien', 'horaire'], col_values=['z_T_p_signif'], nom_fichier="histo_z_T_p")
```
